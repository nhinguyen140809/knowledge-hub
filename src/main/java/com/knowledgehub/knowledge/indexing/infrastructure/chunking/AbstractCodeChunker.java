 ...
 * JavaCodeChunker}) carrying the language-specific parser; the extension is never hard-coded here or
 * in the subclass but read from the registered {@link SourceLanguage}, so it stays the single source
 * of truth. Subclasses register at highest precedence so a code file is chunked by its language
 * strategy before falling through to the document chunker.
 */
public abstract class AbstractCodeChunker implements Chunker {
  ...
  public boolean supports(RawArtifact artifact) {
    return artifact.text() != null
        && artifact.path().toLowerCase(Locale.ROOT).endsWith(extension);
  }
}
