...
\begin{itemize}
  ...
  \item Xây dựng pipeline ingestion: chia dữ liệu thành các chunk (theo AST đối với mã nguồn, theo cấu trúc đối với tài liệu), sinh embedding, rồi xây dựng Knowledge Graph biểu diễn quan hệ giữa các entity (ví dụ: một hàm gọi một hàm khác, một requirement được hiện thực bởi một đoạn code).
...
\begin{itemize}
  ...
  \item Hệ thống không cung cấp giao diện đồ họa (UI); tri thức chỉ được truy vấn qua agent thông qua MCP và REST, còn việc vận hành và cấu hình được thực hiện qua UI của API doc, file cấu hình và Docker.
...
\begin{itemize}
  \item \textbf{AI Agent (bên sử dụng chính):} các agent phục vụ những tác vụ khác nhau trong SDLC (ví dụ viết code, kiểm thử, tạo tài liệu). Agent truy vấn Knowledge Hub qua MCP để lấy ngữ cảnh, và là tác nhân duy nhất tương tác trực tiếp với tri thức tại runtime.
...
\begin{itemize}
  ...
  \item \textbf{Benchmark}: gold set $\geq$ 50 truy vấn có nhãn cùng script đo Recall@10, MRR và latency; và bộ test kiểm chứng phân quyền không rò rỉ.
