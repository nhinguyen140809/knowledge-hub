...
Xuyên suốt các bước, bộ lọc phân quyền là **pre-filter cứng đẩy vào từng kho**: nguồn ngoài quyền đọc bị loại ngay trong truy vấn ở vector store, keyword search và graph traversal, chứ không đợi lọc sau khi kết quả đã rời kho (FR-8.6, NFR-7.4). Bộ lọc source của truy vấn cũng đi qua chính cơ chế này: nó được **giao** với tập nguồn được phép đọc — chỉ thu hẹp phạm vi, không bao giờ mở rộng; khoanh vào một nguồn ngoài quyền đọc cho kết quả rỗng thay vì báo lỗi, nên caller không suy ra được nguồn đó có tồn tại hay không.
