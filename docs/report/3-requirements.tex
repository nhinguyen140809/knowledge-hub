...
\begin{itemize}
  ...
  \item \textbf{FR-1.3} Hệ thống phải nạp dữ liệu từ các source được cấu hình, mỗi source thuộc một trong hai loại: (a) một Git repository (qua remote URL hoặc đường dẫn tới một repo trên đĩa) tại một ref cấu hình được, mặc định là default branch; và (b) một thư mục filesystem không yêu cầu version control. Source loại (a) cung cấp thêm \texttt{ref} và \texttt{commit\_sha} cho provenance (FR-1.4); loại (b) định danh phiên bản qua \texttt{content\_hash}.
...
\begin{itemize}
  \item \textbf{FR-2.1} Hệ thống phải chia dữ liệu thành các chunk trước khi sinh embedding: chunk mã nguồn theo cấu trúc AST (hàm, lớp), chunk tài liệu theo cấu trúc văn bản (heading, đoạn).
  ...
  \item \textbf{FR-2.4} Hệ thống phải lưu trữ đồng thời vector embedding, Knowledge Graph và metadata, sao cho mỗi chunk truy ngược được về nguồn gốc theo FR-1.4.
\end{itemize}

\subsubsection{FR-3: Liên kết tri thức (Knowledge linking)}

\begin{itemize}
  \item \textbf{FR-3.1} Hệ thống phải tạo được các quan hệ cấu trúc và phụ thuộc trong mã nguồn, bao gồm: chứa/khai báo (CONTAINS/DECLARES), gọi hàm (CALLS), nhập phụ thuộc (IMPORTS), kế thừa (EXTENDS), hiện thực interface (IMPLEMENTS) và ghi đè (OVERRIDES).
  \item \textbf{FR-3.2} Hệ thống có thể bổ sung các quan hệ sâu hơn: khởi tạo (INSTANTIATES), đọc/ghi biến, field hoặc constant (READS/WRITES), tham chiếu một định danh khác mà không gọi hàm (REFERENCES), dùng kiểu (HAS\textit{TYPE), annotation (ANNOTATED}WITH), ném ngoại lệ (THROWS), liên kết test (TESTS).
  \item \textbf{FR-3.3} Hệ thống phải liên kết giữa các artifact khác nhau, gồm: tài liệu mô tả code (DESCRIBES), yêu cầu được hiện thực bởi code (IMPLEMENTED\textit{BY), yêu cầu được kiểm bởi test (VERIFIED}BY), commit sửa đổi code (MODIFIES), client gọi tới endpoint/API do code khác cung cấp (CONSUMES), và tài liệu tham chiếu tài liệu khác (LINKS\_TO). Việc liên kết dựa trên các tín hiệu: tên định danh (tên hàm, tên lớp), từ khóa, và tham chiếu đường dẫn; mỗi liên kết suy luận theo heuristic phải kèm một điểm tin cậy (confidence score) để phân biệt liên kết chắc chắn với liên kết phỏng đoán.
...
\begin{itemize}
  ...
  \item \textbf{FR-5.2} Hệ thống phải cung cấp một MCP server expose hai tool: một tool truy vấn tri thức và một tool kích hoạt cập nhật/đồng bộ, để AI Agent vừa tích hợp truy vấn vừa chủ động cập nhật trực tiếp.
...
\begin{itemize}
  ...
  \item \textbf{FR-6.3} Hệ thống phải tránh xử lý trùng lặp bằng dedup theo content hash: chunk có nội dung không đổi thì không sinh lại embedding.
  ...
  \item \textbf{FR-6.6} Hệ thống có thể hỗ trợ thêm cơ chế tự kích hoạt cập nhật như tùy chọn (webhook khi có commit/merge, hoặc quét định kỳ), mặc định tắt.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{FR-7.2} Hệ thống phải cô lập backend lưu trữ vector sau một ranh giới trừu tượng, sao cho việc bổ sung hoặc thay thế backend chỉ đòi hỏi thêm một thành phần lưu trữ tương ứng khi mở rộng.
  \item \textbf{FR-7.3} Hệ thống phải hỗ trợ cấu hình theo nhiều môi trường (dev, server, cloud).
\end{itemize}
...
\begin{itemize}
  ...
  \end{itemize}
  \item \textbf{FR-8.5} Trên mỗi request, hệ thống phải resolve credential đã xác thực (FR-8.2) về đúng một principal. Quyền hiệu lực của một principal là hợp (union) quyền của chính nó và mọi \texttt{group} chứa nó (đệ quy).
  \item \textbf{FR-8.6} Hệ thống phải áp ACL như một filter cứng trước khi kết quả rời hệ thống, trên tất cả đường truy xuất: semantic search, keyword search, graph traversal và fusion. Kết quả, node/quan hệ kề trong graph, và metadata có nguồn gốc từ source mà principal không được đọc không được xuất hiện trong phản hồi.
...
\begin{itemize}
  \item \textbf{NFR-3.1} Hệ thống phải phục vụ được kho $\geq$ 2 triệu chunk mà vẫn đạt các ngưỡng ở NFR-1 và NFR-2.
  \item \textbf{NFR-3.2} Khi mở rộng theo chiều ngang bằng cách bổ sung hoặc thay thế backend lưu trữ (FR-7.2), hệ thống phải chạy đúng và vượt qua toàn bộ test chức năng.
  \item \textbf{NFR-3.3} Với $\geq$ 20 truy vấn đồng thời, độ trễ p95 không được vượt quá 2 lần so với khi tải đơn.
\end{itemize}

\subsubsection{NFR-4: Hiệu quả lưu trữ (Storage efficiency)}

\begin{itemize}
  \item \textbf{NFR-4.1} Khi index thêm một branch chỉ khác default branch dưới 10\% số file, dung lượng tăng thêm phải dưới 15\% (nhờ dedup theo content hash).
  \item \textbf{NFR-4.2} Khi bật tùy chọn nén/quantization, dung lượng vector phải giảm $\geq$ 50\% với mức giảm Recall@10 $\leq$ 2 điểm phần trăm.
\end{itemize}

\subsubsection{NFR-5: Hiệu quả cập nhật \& độ mới (Update efficiency / Freshness)}

\begin{itemize}
  \item \textbf{NFR-5.1} Cập nhật incremental cho một commit ($\leq$ 50 file thay đổi) phải hoàn tất trong $\leq$ 2 phút.
\end{itemize}

\subsubsection{NFR-6: Độ tin cậy \& ổn định (Reliability)}

\begin{itemize}
  \item \textbf{NFR-6.1} Khi nạp lỗi một file hoặc nguồn, hệ thống phải bỏ qua phần lỗi, giữ nguyên chỉ mục hiện có và ghi log; kiểm chứng bằng test inject lỗi.
  \item \textbf{NFR-6.2} Lệnh gọi phụ thuộc bên ngoài (embedding provider) phải retry tối thiểu 3 lần kèm backoff trước khi báo lỗi; mọi lỗi phải được log.
  \item \textbf{NFR-6.3} Trong khi đang re-index, tỉ lệ truy vấn thành công phải $\geq$ 99\% (không khóa toàn bộ).
\end{itemize}

\subsubsection{NFR-7: Bảo mật \& quyền riêng tư (Security \& privacy)}

\begin{itemize}
  \item \textbf{NFR-7.1} 100\% endpoint REST và MCP phải yêu cầu xác thực (FR-8.2); request không có khóa hợp lệ phải bị từ chối.
  \item \textbf{NFR-7.2} Không được hard-code bất kỳ secret nào (API key, mật khẩu DB) trong mã nguồn; kiểm chứng bằng quét secret.
  \item \textbf{NFR-7.3} Ở chế độ local model, không request nào chứa dữ liệu nguồn được rời mạng nội bộ; kiểm chứng bằng giám sát lưu lượng mạng.
  \item \textbf{NFR-7.4} Với một principal không có quyền đọc source S, 0\% kết quả truy xuất -- kể cả kết quả đến từ graph traversal và sau fusion -- có nguồn gốc từ S; kiểm chứng bằng bộ test phủ từng đường truy xuất (semantic, keyword, graph).
  \item \textbf{NFR-7.5} Việc áp ACL không được làm tăng độ trễ p95 của truy vấn quá 10\% so với cùng truy vấn khi không áp ACL, trên cùng kho và cùng mức tải.
  \item \textbf{NFR-7.6} Mọi thay đổi config runtime (ACL, danh sách source, tunable) phải có hiệu lực với mọi request nhận sau thời điểm thay đổi được ghi nhận mà không cần restart, với độ trễ áp dụng $\leq$ 5 giây; kiểm chứng bằng test thay đổi rồi truy vấn lại.
\end{itemize}

\subsubsection{NFR-8: Tính dễ dùng của API (API usability)}

\begin{itemize}
  \item \textbf{NFR-8.1} 100\% endpoint phải có mô tả OpenAPI kèm ví dụ request/response (FR-5.3).
\end{itemize}

\subsubsection{NFR-9: Đóng gói \& triển khai (Deployability \& portability)}

\begin{itemize}
  \item \textbf{NFR-9.1} Mọi thành phần của hệ thống phải có image Docker.
  \item \textbf{NFR-9.2} Toàn hệ thống phải khởi chạy bằng một lệnh \texttt{docker compose up} và sẵn sàng nhận request trong $\leq$ 5 phút trên máy chuẩn.
  \item \textbf{NFR-9.3} Chuyển giữa các môi trường (dev, server, cloud) chỉ bằng thay đổi cấu hình, không build lại image; kiểm chứng bằng việc chạy đúng ở cả ba.
\end{itemize}

\subsubsection{NFR-10: Khả năng bảo trì (Maintainability)}

\begin{itemize}
  \item \textbf{NFR-10.1} Thay embedding provider chỉ bằng thay đổi cấu hình (FR-7.1); tiêu chí: chuyển đổi thành công và vượt qua bộ test chức năng.
  \item \textbf{NFR-10.2} Hệ thống phải log mọi sự kiện ingestion, truy vấn và lỗi; tiêu chí: tái dựng lại được một request lỗi chỉ từ log.
  \item \textbf{NFR-10.3} Thêm một định dạng tài liệu mới hoặc một nguồn dữ liệu mới chỉ cần bổ sung một thành phần xử lý độc lập; kiểm chứng bằng việc thêm một parser mẫu.
\end{itemize}
