...
*Ví dụ — một truy vấn đi qua ba tầng.* Request `POST /query` chạm vào controller (hạ tầng, adapter inbound), được chuyển thành đối tượng truy vấn của miền rồi giao cho service truy xuất (ứng dụng). Service điều phối trình tự — sinh embedding, tìm kiếm, hợp nhất, lọc — nhưng chỉ gọi qua các *cổng* miền; những cổng đó được nối tại runtime tới adapter Qdrant, Neo4j và API embedding (hạ tầng, adapter outbound). Muốn thay engine vector chỉ cần viết một adapter mới cho cùng cổng: controller, service và mô hình miền không đổi một dòng — và cũng chính vì service chỉ biết cổng, toàn bộ luồng truy vấn kiểm thử được bằng adapter giả, không cần dựng cơ sở dữ liệu thật.
