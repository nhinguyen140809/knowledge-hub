...
- **Tool liệt kê nguồn** — cho agent khám phá các source nó được phép đọc, mỗi source kèm id, loại (git hay filesystem), ref cấu hình, và tên cùng mô tả người-đọc-được nói nguồn chứa gì (để agent nhận ra và chọn đúng nguồn). Tập trả về chính là tập mà mọi truy vấn bị giới hạn vào, nên tool không lộ gì mà truy vấn không thể lộ; agent mới kết nối gọi tool này trước tiên để biết mình có thể hỏi về những gì.
...
Ngoài id, loại và vị trí, mỗi nguồn còn mang **tên** và **mô tả** người-đọc-được tùy chọn — một nhãn ngắn và một câu giải thích nguồn chứa gì — đặt khi đăng ký và sửa được về sau; chúng không đổi hành vi mà chỉ giúp người vận hành lẫn agent nhận ra nguồn.

Ngữ nghĩa cập nhật được chọn có chủ đích ở những chỗ dễ nhầm. Sửa cấu hình nguồn là **cập nhật một phần** (partial update): trường không gửi thì giữ nguyên giá trị hiện tại, mảng rỗng nghĩa là xóa danh sách, mảng có phần tử là thay thế toàn bộ, và tên/mô tả để trống thì xóa. *Ví dụ:* gửi `{"ignore": ["build"]}` chỉ thay danh sách loại trừ, còn ref và danh sách include giữ nguyên — người gọi không phải chép lại toàn bộ cấu hình chỉ để sửa một trường.
