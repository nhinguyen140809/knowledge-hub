...
- **Quan hệ sâu** (FR-3.2) — cũng tất định nhưng chi tiết hơn và tốn kém hơn, nên là lựa chọn bật thêm: khởi tạo (INSTANTIATES), đọc/ghi trường (READS/WRITES), tham chiếu định danh (REFERENCES), dùng kiểu (HAS_TYPE), annotation (ANNOTATED_WITH), ném ngoại lệ (THROWS), liên kết test (TESTS).

- **Quan hệ liên artifact** (FR-3.3) — nối các loại tri thức khác nhau: tài liệu mô tả mã (DESCRIBES), yêu cầu được hiện thực bởi mã (IMPLEMENTED_BY), yêu cầu được kiểm bởi test (VERIFIED_BY), commit sửa đổi mã (MODIFIES), lời gọi API giữa các service (CONSUMES), tài liệu tham chiếu tài liệu (LINKS_TO). Khác hai nhóm trên, các quan hệ này **suy luận theo heuristic** nên bắt buộc kèm một **điểm tin cậy** (confidence score).
